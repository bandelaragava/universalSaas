...
export function Receipt() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        try {
          const resConf = await rolesApi.get('/invoice-configurations/active?module=VENDOR');
  ...
  const invoiceRef = invoice.invoiceNumber 
    ? `${config?.invoicePrefix || ''}${invoice.invoiceNumber}` 
    : `${config?.invoicePrefix || 'INV-'}${invoice.id}`;
