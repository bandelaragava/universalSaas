...
state_repl = """  const [products, setProducts] = useState<any[]>([]);
...
"""
content = content.replace("  const openAssignModal = (product: any) => {", lifecycle_funcs + "\n  const openAssignModal = (product: any) => {")
                  ...
                  deployedAssets.map((asset, index) => (
                    <tr key={index} className="hover:bg-slate-700/30 transition-colors">
                      <td className="p-4 font-medium text-slate-200">
                        {asset.assetIdentifier || `Asset #${asset.id}`}
                      </td>
                      <td className="p-4">
                        <span className={`px-2.5 py-1 text-xs rounded-full border ${asset.itemType === 'CONSUMABLE' ? 'bg-indigo-500/10 text-indigo-400 border-indigo-500/30' : 'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/30'}`}>
                      ...
                      <td className="p-4 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => openAssetHistory(asset)} className="text-slate-400 hover:text-cyan-400 px-2 py-1 transition-colors">History</button>
                          
                          {asset.status === 'ASSIGNED' && asset.itemType === 'ASSET' && (
                            <>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'damage')} className="text-rose-400 hover:text-rose-300">Damage</button>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'return')} className="text-amber-400 hover:text-amber-300">Return</button>
                            </>
                          )}
                          ...
                          {asset.status === 'DAMAGED' && asset.itemType === 'ASSET' && (
                            <>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'repair')} className="text-amber-400 hover:text-amber-300">Repair</button>
                            </>
                          )}
                          ...
                          {asset.status === 'UNDER_REPAIR' && asset.itemType === 'ASSET' && (
                            <>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'complete-repair')} className="text-emerald-400 hover:text-emerald-300">Complete Repair</button>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'not-repairable')} className="text-rose-400 hover:text-rose-300">Scrap</button>
                            </>
                          )}
                          ...
                          {asset.status === 'ASSIGNED' && asset.itemType === 'CONSUMABLE' && (
                            <>
                              <span className="text-slate-600">|</span>
                              <button onClick={() => openLifecycleModal(asset, 'consume')} className="text-indigo-400 hover:text-indigo-300">Consume</button>
                            </>
                          )}
                        </div>
                      </td>
...
history_modal_old = """                  {loadingHistory ? (
...
history_modal_new = """                  {loadingHistory ? (
...
content = content.replace(history_modal_old, history_modal_new)
...
content = content.replace("Total Assigned: {selectedProduct?.assignedQuantity}", "Total Assigned: {selectedProduct?.quantity}")
...
with open(r"C:\Users\ASUS\Downloads\universalSaas\universalSaas\src\pages\vendor\ReceivedProducts.tsx", "w", encoding="utf-8") as f:
    f.write(content)
print("Updated ReceivedProducts UI")
